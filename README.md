...
The JSON schema is the contract between the two halves, and each half is a package. `packages/form-generator` renders a document and imports nothing from the builder; `packages/builder` edits one — the drag-and-drop, the stores and the preview panes. `apps/example` is the five files that wire them together, one consumer among however many you write. See [Layout](#layout).
